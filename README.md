...
If you're [using WriteIt](#integrating-with-writeit) then the next step is to **refresh the data source in WriteIt so it knows about the new people you've just added**. Once the refresh has happened it's a good idea to check it was successful by seeing if you can write to one of the newly added councillors in the frontend.
