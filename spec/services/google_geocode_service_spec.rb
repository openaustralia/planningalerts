...
describe "GoogleGeocodeService" do
  ...
  context "street address not in australia" do
    ...
    let(:geokit_result) do
      double(success: true, all: [double(lat: 1, lng: 2, country_code: "US", city: "New York", state: "NY", zip: nil, full_address: "New York, NY", accuracy: nil)])
    end
  ...
  context "address with multiple matches" do
    ...
    let(:geokit_result) do
      double(
        success: true,
        all: [
          double(country_code: "AU", lat: 1, lng: 2, accuracy: 6, city: "Glenbrook", state: "NSW", zip: nil, full_address: "Bruce Road, Glenbrook, NSW, Australia"),
          double(country_code: "AU", lat: 1.1, lng: 2.1, accuracy: 6, city: "Somewhere else", state: "VIC", zip: nil, full_address: "Bruce Road, Somewhere else, VIC, Australia")
        ]
      )
    end
  ...
  context "another address with multiple matches" do
    ...
    let(:geokit_result) do
      double(
        success: true,
        all: [
          double(full_address: "Bathurst Rd, Orange NSW 2800, Australia", country_code: "AU", lat: nil, lng: nil, city: "Orange", state: "NSW", zip: "2800", accuracy: 5, success: true),
    ...
    let(:geokit_result) do
      double(
        ...
        all: [
          ...
          double(full_address: "Sowerby St, Garfield NSW 2580, Australia", country_code: "AU", lat: nil, lng: nil, city: "Garfield", state: "NSW", zip: "2580", accuracy: 5, success: true),
