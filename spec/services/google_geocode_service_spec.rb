...
describe "GoogleGeocodeService" do
  ...
  context "when google returns an unexpected status" do
    let(:address) { "24 Bruce Road, Glenbrook, NSW 2773" }
    ...
    before do
      response = instance_double(
        ...
        parsed_response: {
          ...
          "error_message" => "This is a synthetic error message"
        }
      )
    ...
    it "reports the google status and error message to Sentry" do
      allow(Sentry).to receive(:capture_exception)
      result
      expect(Sentry).to have_received(:capture_exception).with(
        an_instance_of(GoogleGeocodeService::GoogleGeocodeError)
          .and(having_attributes(message: "Google geocoding error: REQUEST_DENIED")),
        extra: { status: "REQUEST_DENIED", error_message: "This is a synthetic error message" }
      )
    end
  end
