...
describe ApiController do
  ...
  describe "#area" do
    ...
    it "should find recent applications in an area" do
      ...
      expect(Application).to receive(:with_current_version).and_return(scope1)
      expect(scope1).to receive(:order).and_return(scope2)
      expect(scope2).to receive(:where).with("application_versions.lat > ? AND application_versions.lng > ? AND application_versions.lat < ? AND application_versions.lng < ?", 1.0, 2.0, 3.0, 4.0).and_return(scope3)
      expect(scope3).to receive(:includes).and_return(scope4)
      expect(scope4).to receive(:paginate).with(page: nil, per_page: 100).and_return(result)
  ...
  describe "#date_scraped" do
    ...
    context "valid authentication" do
      ...
      before(:each) do
        5.times do
          create_geocoded_application(date_scraped: Time.utc(2015, 5, 5, 12, 0, 0))
          create_geocoded_application(date_scraped: Time.utc(2015, 5, 6, 12, 0, 0))
        end
      end
