...
class CreateOrUpdateApplicationService
  ...
  def call
    Application.transaction do
      ...
      if application
        ...
        application = Application.create!(attributes.symbolize_keys.merge(authority:, council_reference:, first_date_scraped: attributes["date_scraped"]))
      end
  ...
  def create_version(application)
    ...
    new_version.assign_attributes(new_version.geocode_attributes) unless new_version.lat && new_version.lng && new_version.suburb && new_version.state && new_version.postcode
    new_version.save!
