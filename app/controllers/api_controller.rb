...
class ApiController < ApplicationController
  ...
  sig { void }
  def suburb_postcode
    apps = Application.with_current_version.order("application_versions.date_scraped DESC")
    descriptions = []
    if params[:suburb]
  ...
  def area
    ...
    api_render(
      Application.with_current_version.order("application_versions.date_scraped DESC").where("lat > ? AND lng > ? AND lat < ? AND lng < ?", lat0, lng0, lat1, lng1),
