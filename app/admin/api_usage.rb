...
ActiveAdmin.register_page "API usage" do
  ...
  content do
    ...
    table_for result do
      column(:name) {         |usage| link_to usage.api_key_object.user.name,         nimda_user_path(usage.api_key_object.user) }
      column(:organisation) { |usage| link_to usage.api_key_object.user.organisation, nimda_user_path(usage.api_key_object.user) }
      column(:email) {        |usage| link_to usage.api_key_object.user.email,        nimda_user_path(usage.api_key_object.user) }
